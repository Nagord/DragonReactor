...
namespace DragonReactor.WarpDriveProgram
{
        ...
        public static PLWarpDriveProgram CreateWarpDriveProgram(int Subtype, int level)
        {
            ...
            if (Subtype >= Instance.VanillaWarpDriveProgramMaxType)
            {
                ...
                if (subtypeformodded <= Instance.WarpDriveProgramTypes.Count && subtypeformodded > -1)
                {
                    ...
                    InWarpDriveProgram.GetType().GetField("ShieldBooster_ActiveTime", BindingFlags.Instance | BindingFlags.NonPublic).SetValue(InWarpDriveProgram, WarpDriveProgramType.ActiveTime);
    ...
    class WarpDriveProgramFinalLateAddStatsPatch
    {
        static void Postfix(PLWarpDriveProgram __instance)
        {
            ...
            if (subtypeformodded > -1 && subtypeformodded < WarpDriveProgramPluginManager.Instance.WarpDriveProgramTypes.Count && Time.time - (float)__instance.GetType().GetField("ShieldBooster_LastActivationTime", BindingFlags.Instance | BindingFlags.NonPublic).GetValue(__instance) < (float)__instance.GetType().GetField("ShieldBooster_ActiveTime", BindingFlags.Instance | BindingFlags.NonPublic).GetValue(__instance))
            {
    ...
    class WarpDriveProgramExecuteBasedOnTypePatch
    {
        static void Prefix(PLWarpDriveProgram __instance)
        {
            ...
            {
                if (WarpDriveProgramPluginManager.Instance.WarpDriveProgramTypes[subtypeformodded].IsVirus) //work on this. Virus Subtype seems to be off
                {
